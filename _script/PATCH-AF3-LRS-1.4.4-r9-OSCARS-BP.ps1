...
$PatchVersion = "1.4.4-r9-oscars-bp"
...
function Check-IconFiles {
    ...
    $bp = @(Get-ChildItem -LiteralPath $skinRoot -Recurse -Filter "oscars_bp.png" -ErrorAction SilentlyContinue)
    ...
    Write-Host "  oscars_bp.png : $($bp.Count) found"
    if ($bp.Count -eq 0) {
        Write-Host "[WARN] oscars_bp.png belum ditemukan di skin. Letakkan oscars_bp.png di folder ratings yang aktif, misalnya:"
        Write-Host "       E:\Kodi\portable_data\addons\skin.arctic.fuse.3\media\flags\<color>\ratings\oscars_bp.png"
    }
}
